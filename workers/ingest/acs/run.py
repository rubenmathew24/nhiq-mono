"""Census ACS 5-year tract ingest — fixture counties only."""
...
from ingest.acs.client import DEFAULT_ACS_YEAR, fetch_county_tract_rows, tabular_to_dicts
...
INSERT INTO acs_indicators (
    ...
    %(employed)s, %(unemployed)s, %(acs_year)s, %(payload)s
)
...
class AcsTractWorker(BaseIngestionWorker):
    ...
    def fetch(self) -> None:
        self._raw_rows = []
        for county_fips in sorted(fixture_county_fips()):
