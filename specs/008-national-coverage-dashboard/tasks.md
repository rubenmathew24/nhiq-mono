...
- [x] T010 [US3] Add per-state breakdown (`states[]`) with per-source stats scoped to each state’s registry counties in `coverage_service.py`
- [x] T011 [P] [US3] Add by-state assertion test in `apps/api/tests/test_coverage_service.py`

## Phase 6: Web page

- [x] T012 [US1] Add TypeScript coverage types in `apps/web/src/types/api.ts`
- [x] T013 [US1] Create public `apps/web/src/app/coverage/page.tsx` with overall / by source / by state views (no auth; do not touch middleware auth for `/dashboard`)
