...
- **SC-014**: Press-and-drag text selection inside a category box (e.g. summary or expand value) succeeds and does not toggle expand; a simple click without a drag still toggles.
...
### Session 2026-07-16 (UX polish round 4 — text selection)

- Category boxes remain click-to-expand, but users MUST be able to **drag-select text** inside the box without toggling expand (remove `select-none`; ignore click when pointer moved or a selection exists).
