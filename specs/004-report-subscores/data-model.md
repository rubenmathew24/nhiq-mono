...
| `acs_indicators` + `bls_laus_county` | Income, unemployment, **employment rate** (employed/labor_force) |
...
```json
{
  "healthcare": {
    ...
    "stats": [
      ...
      {"name": "2nd nearest ER", "value": "Hospital B · 4.0 mi · ★3", "impact": "neutral", "tone_score": 55},
  ...
  "safety": {
    ...
    "stats": [
      {"name": "Violent crime vs state", "value": "About 0.9× the state benchmark", "impact": "positive"},
      {"name": "Homicide", "value": "…", "impact": "neutral"},
      {"name": "Robbery", "value": "…", "impact": "neutral"},
      {"name": "Assault", "value": "…", "impact": "neutral"},
  ...
  "education": {
    ...
    "stats": [
      ...
      {"name": "Nearest middle", "value": "Example Middle · 1.2 mi", "impact": "positive"},
      {"name": "Nearest high", "value": "Example High · 2.0 mi", "impact": "neutral"}
    ]
  },
  "environment": {
    "sub_scores": [],
...
- Schools stats: one row per available level bucket; omit levels with no nearby school; never include pupil–teacher or locale.
...
See [research.md](./research.md) §3. Category numeric columns remain the weighted blend written by the score job. Staffing may be unavailable without changing published overall weights beyond renormalizing education when staffing is dropped.
...
NeighborhoodScore.score_detail ← scoring reads hospitals, schools (by level),
  crime, ACS (incl. employed/labor_force), LAUS, EPA/OM, FemaNri, Timely
```
