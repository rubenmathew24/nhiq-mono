...
- **FR-006a**: National census ingest MUST persist TIGER land area and water area (`aland` / `awater`, m²) on `census_tracts` consistently with [`002-data-ingestion-workers`](../002-data-ingestion-workers/) FR-004a. Checkpoint “census done” (inventory, national status, and `/coverage`) MUST require that every tract in the county has **non-NULL** `aland` (water-only `aland = 0` counts as complete). Counties with tracts but any `aland IS NULL` MUST appear as census gaps so continuous national ingest re-runs census **without** requiring `INGEST_FORCE`. Water-only tracts MUST still be stored (not deleted) so county and report-detail coverage stay complete.
