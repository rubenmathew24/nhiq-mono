...
| Bounded | `ORCH_CONTINUOUS=0` / false | Existing max_states / filter / force / exclude semantics; class A before B for unscoped gap-fill |

## Batching

For each pipeline worker in order, select up to `ORCH_BATCH_STATES` (default `10`) gap states and start **one** ACA execution with `INGEST_STATE_BATCH=SS,SS,...`. Prefer class A (report-detail backfill) before class B when building the gap set.
