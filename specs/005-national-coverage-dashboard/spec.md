...
**Feature Branch**: `005-national-coverage-dashboard`

**Historical branch**: Delivered on git branch `007-national-ingest-redesign` (national ingest redesign era; Spec Kit folder was formerly `008-national-coverage-dashboard`).

**Created**: 2026-07-17

**Status**: Draft

**Input**: User description: "Add a public dashboard page on the site that shows how much of the national data has been loaded — overall and by state (with a source filter including overall) — with no auth. Use correct expected denominators from the national ingest redesign. Deliver via Spec Kit (historical git branch `007-national-ingest-redesign`; feature dir now `005-national-coverage-dashboard`)."
...
- Historical delivery: git branch `007-national-ingest-redesign`; Spec Kit feature directory is `005-national-coverage-dashboard`.
...
- Completion rules for coverage/status display align with `003-national-ingest` national status semantics (including scoring county grain, EPA monitors, Urban÷NCES), with Overall ↔ By state parity required.
